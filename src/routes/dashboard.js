...
router.get('/dashboard',estaLogeado,async(req,res)=>{//Creando una ruta llamada /dashboard para mostrar la interfaz metodo get
    ...
    const miCocina = await pool.query('SELECT * FROM inventario WHERE nombre_inv = ? and id_usuario = ?',['Mi Alacena',req.user.id_usuario]);//Seleccionamos el invenatrio comun Mi Cocina
    const id_usuario = req.user.id_usuario;

    listas = inventarios.filter(inventario => inventario.nombre_inv != "Mi Alacena");
    
    //console.log('Estas son las listas del usuario ' + id_usuario + ': ' + listas);
...
router.get('/dashboard/eliminar/:id_inventario',estaLogeado,async(req,res)=>{//RUTA PARA ELIMINAR LOS INVENTARIOS
    ...
    req.flash('success','Evento eliminado exitosamente');
...
router.get('/dashboard/lista/remover/productos/:id_inventario',async(req,res)=>{
    ...
    const miCocina = await pool.query('SELECT *FROM inventario WHERE nombre_inv = ? and id_usuario = ?',['Mi Alacena',req.user.id_usuario]);
